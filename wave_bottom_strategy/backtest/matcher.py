...
class Order:
    ...
    target_price: float = 0.0  # 目标价格?表示市价?
...
class OrderMatcher:
    ...
    def create_order(
        ...
        logger.info(f"创建订单: {direction} {ts_code} {shares}?@ {order_date}")
    ...
    def process_orders(
        ...
        for order in self.pending_orders:
            ...
            logger.info(f"订单成交: {order.direction} {order.ts_code} {order.shares}?@ {actual_price:.2f}")
