...
def run_backtest(start_date: str, end_date: str, initial_capital: float = 1000000):
    ...
    logger.info(f"沪深300成分�? {len(stock_pool)}�?)
    
    # 过滤ST和停�?
    filtered_pool = processor.filter_stocks(stock_pool, exclude_st=True)
    logger.info(f"过滤�? {len(filtered_pool)}�?)
    ...
    result = engine.run(
        ...
        end_date=end_date,
        stock_pool=filtered_pool[:50]  # 简化：取前50�?
    )
    ...
    logger.info(f"报告已生�? {report_path}")
    ...
    logger.info("=" * 50)
    logger.info(f"初始资金: {initial_capital:,.0f}")
    logger.info(f"最终净�? {result.get('final', 0):,.0f}")
...
def run_analysis(result: dict):
    ...
    logger.info(f"盈亏�? {metrics.profit_loss_ratio():.2f}")
    logger.info(f"夏普比率: {metrics.sharpe_ratio():.2f}")
    logger.info(f"最大回�? {metrics.max_drawdown():.2%}")
...
def recommend_capital(risk_level: str = 'medium'):
    ...
    logger.info(f"  最低资�? {s['min']:,.0f}")
    logger.info(f"  推荐资金: {s['recommended']:,.0f}")
    logger.info(f"  最高资�? {s['max']:,.0f}")
...
def main():
    ...
    parser.add_argument('--start', default='2020-01-01', help='开始日�?)
